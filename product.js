...
$(document).ready(function () {
    $.get(url, function (response) {
        for (var i = 0; i < response.length; i++) {
            handleRowCreation(checkedFilters, response[i]);
        }
        displayCount();
...
// Function to display the row count
...
// Function to check if the product is in low stock
function checkLowStock(data) {
    return data.stock < 100;
}

// Function to handle row creation based on filters
function handleRowCreation(filterArr, data) {
    switch (checkedFilters.length) {
        case 2:
            ...
            break;
        case 1:
            if (filterArr.includes("Expired") && !checkLowStock(data)) {
                $("#order-table").append(createRow(data));
            } else if (filterArr.includes("Low Stock") && !checkExpiry(data)) {
                $("#order-table").append(createRow(data));
            }
            break;
        default:
